...
const drawQuoteContentOnPage = async (
  ...
  // Included Features section (professional layout)
  const featuresStartY = tableY - 120;
  
  page.drawText(sanitizeText('Included Features'), {
    x: leftColumn,
    y: featuresStartY,
    size: 18,
    font: helveticaBold,
  ...
  // Features list using actual quote features
  let featuresY = featuresStartY - 25;
  const featuresPerRow = 2;
  const featureSpacing = 200;
  
  quote.selectedTier.features.forEach((feature, index) => {
    const row = Math.floor(index / featuresPerRow);
    const col = index % featuresPerRow;
    const x = leftColumn + (col * featureSpacing);
    const y = featuresY - (row * 20);
    ...
    // Draw feature text
    page.drawText(sanitizeText(feature), {
      x: x + 15,
      y: y,
      size: 11,
      font: helveticaFont,
