...
const HubSpotIntegration: React.FC<HubSpotIntegrationProps> = ({ 
  ...
  const fetchHubSpotContacts = async () => {
    ...
    try {
      ...
      try {
        ...
        response = await fetch('http://localhost:3001/api/hubspot/contacts', {
          method: 'GET',
          headers: {
  ...
  const fetchHubSpotDeals = async () => {
    ...
    try {
      ...
      try {
        ...
        response = await fetch('http://localhost:3001/api/hubspot/deals', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  useEffect(() => {
    console.log('🚀 HubSpotIntegration component loaded, attempting auto-connection...');
    ...
    // Only auto-connect if not already connected and not connecting
    if (!state.isConnected && !state.isConnecting) {
      console.log('🔄 Auto-connecting to HubSpot...');
      // Add a small delay to ensure component is fully mounted
      setTimeout(() => {
        testHubSpotConnection();
      }, 100);
    } else {
      console.log('⏭️ Skipping auto-connection - already connected or connecting');
    }
  ...
  const testHubSpotConnection = async () => {
    ...
    try {
      ...
      console.log('🌐 Backend URL: http://localhost:3001/api/hubspot/test');
      
      const backendResponse = await fetch('http://localhost:3001/api/hubspot/test', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
