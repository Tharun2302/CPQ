...
app.post('/api/quotes', async (req, res) => {
  try {
    ...
    await connection.execute(`
      INSERT INTO quotes (id, client_name, client_email, company, configuration, selected_tier, calculation, status)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      ...
      selected_tier = VALUES(selected_tier),
      calculation = VALUES(calculation),
      status = VALUES(status),
      updated_at = CURRENT_TIMESTAMP
    `, [id, clientName, clientEmail, company, JSON.stringify(configuration), JSON.stringify(selectedTier), JSON.stringify(calculation), status]);
    ...
    const quotes = rows.map(row => {
      try {
        return {
          ...
          selectedTier: row.selected_tier ? JSON.parse(row.selected_tier) : {},
          calculation: row.calculation ? JSON.parse(row.calculation) : {}
        };
      } catch (parseError) {
        console.warn('JSON parse error for quote ID:', row.id, parseError.message);
        return {
          ...row,
          configuration: {},
          selectedTier: {},
